    ...
    <header className="relative mb-10 xl:mb-0">
      <div className="pointer-events-none absolute left-[max(-0.5rem,calc(50%-18.625rem))] top-0 z-50 flex h-4 items-center justify-end gap-x-2 lg:left-0 lg:right-[calc(max(2rem,50%-38rem)+40rem)] lg:min-w-[32rem] xl:h-8">
        <Link href={`#${id}`} className="inline-flex">
          ...
          /> */}
          <h1 className="hidden xl:pointer-events-auto xl:block font-medium xl:text-2xs/4 xl:font-medium xl:text-gray-900/50">{title}</h1>
        </Link>
      ...
      <ContentWrapper>
        <div className="flex">
          <Link href={`#${id}`} className="inline-flex">
            ...
            /> */}
            <h1 className="text-2xs/4 font-medium text-gray-600 xl:hidden">{title}</h1>
          </Link>
        </div>
      </ContentWrapper>
    </header>
